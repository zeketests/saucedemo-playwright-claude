...
| CART-07 | Cart persists items after logout and re-login | Negative | Cart should be empty after new session |

---

### 5. Checkout — Step 1: Customer Information
...
| Suite | Priority | Risk | Automatable |
|-------|----------|------|-------------|
| Authentication | Critical | High | Yes |
| Inventory / Sorting | High | Medium | Yes |
| Shopping Cart | High | High | Yes |
| Checkout Step 1 (validation) | High | High | Yes |
| Checkout Step 2 (price math) | High | High | Yes |
| Order Confirmation | Medium | Medium | Yes |
| Product Detail | Medium | Low | Yes |
| Navigation & Session | Medium | Medium | Yes |
...
pages/
├── LoginPage.ts          ✅ exists
├── InventoryPage.ts      ✅ exists
├── ProductDetailPage.ts  ⬜ to create
├── CartPage.ts           ⬜ to create
├── CheckoutInfoPage.ts   ⬜ to create
├── CheckoutOverviewPage.ts ⬜ to create
└── OrderConfirmationPage.ts ⬜ to create
```
